...
页面支持图片/PDF 选择、拖拽上传、截图后 `Ctrl+V` 粘贴、粘贴后自动识别、识别后自动复制、多图/PDF 批量识别、启发式表格还原、代码块还原、结果搜索/高亮、细粒度加载阶段提示、Canvas 标框、文本复制、JSON/CSV 导出。
...
3. 默认开启“粘贴后自动识别”。
4. 可按需开启“识别后自动复制”。
5. 单张图片或 PDF 页面可点“识别”，多张图片或多页 PDF 可点“识别全部”。识别后可搜索当前页面结果、复制纯文本、还原表格、还原代码块、导出 JSON 或 CSV。
...
- 点击“还原表格”会根据 OCR 文本框坐标把内容按行列整理，并在底部文本区显示 Markdown 表格。
- 点击“复制表格”会复制 Markdown 表格。
- 点击“导出 CSV”会导出当前图片或 PDF 页面的表格结果。
- JSON 导出会额外包含 `table` 字段，里面有 `columns`、`rows`、`markdown` 和 `csv`。

当前表格还原是启发式算法：优先用第一行多列内容作为列锚点，再按最近列归位，适合规则截图表格和树形缩进字段表。复杂合并单元格、跨页表格、无明显列对齐的版面仍需要专门的表格结构模型。

## 代码块还原

- 点击“还原代码”会根据 OCR 文本框坐标按行恢复代码，并尝试删除左侧行号列。
- 点击“复制代码块”会复制 Markdown fenced code block，并自动尝试添加 `json` 等语言标记。
- JSON 导出会额外包含 `code` 字段，里面有 `language`、`text`、`markdown` 和 `lines`。

代码块还原适合接口文档、JSON 示例、配置片段等截图。它会优先删除左侧连续数字行号列，并按 x 坐标估算缩进；如果截图里同时包含 OCR 标号、复杂换行、跨列注释或代码被严重拆碎，仍可能需要人工微调。
